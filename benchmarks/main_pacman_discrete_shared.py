...
WORKDIR = '/home/adith/simulator_sniper/sniper3/benchmarks'
print "RUNNING SINGLE NETWORK MULTI-SCORE ACTOR CRITIC (LOCAL)"
sys.path.append(WORKDIR)


import pacman_power_checker
...
if 'fft_model_discrete.pth' in os.listdir(WORKDIR):
    print "loading previously saved network"
    model.load_state_dict(torch.load(WORKDIR + '/fft_model_discrete.pth'))
...
class RLAgent:
  ...
  def run_local_RL_agent(self, time, time_delta, debug=False):
      ...
      new_weights = []
      for core in range(sim.config.ncores):
          state  = self.get_state(time, time_delta, core, debug=True, scale=True)
  ...
  def finish_episode(self):
      ...
      for r in model.rewards[::-1]:
          if iteration_number % sim.config.ncores == 0:
              R = r + self.gamma * R
          else:
              R = r + R
      ...
      torch.save(model.state_dict(), WORKDIR + '/fft_model_discrete.pth')
