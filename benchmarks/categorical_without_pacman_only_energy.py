...
import network_categorical_without_pacman as single_pacman_network
...
### VERY IMPORTANT
### ALWAYS CREATE A NEW MODEL_NAME
models     = []
optimizers = []
model_name = 'network-energy-only-four-core-{}-{}.pt'
for core in range(sim.config.ncores):
    print 'Creating an actor-critic model for core number: ' + str(core)
    models.append(single_pacman_network.Policy())
    if model_name.format(CURR_BENCHMARK, core) in os.listdir(WORKDIR): # if model exists
...
class RLAgent:
  ...
  def setup(self, args):
      ...
      self.Q        = int(q_interval_ns / interval_ns) # smallest value
      self.M        = int(g_interval_ns / interval_ns) # medium value
      self.K        = int(25 * self.Q) #number of iterations before performing backprop # largest value
