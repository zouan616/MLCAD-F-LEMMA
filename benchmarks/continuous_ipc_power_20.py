...
models     = []
optimizers = []
model_name = 'continuous_power_ipc_swift-reward20{}-{}.pt'
...
class RLAgent:
  ...
  def get_reward(self, time, time_delta, core, debug=False):
      ...
      print "DELTA ENERGY IN REWARD FUNCTION.{}:".format(core), curr_delta_energy
      self.prev_energies[core] = current_energy
      reward = -1 * curr_delta_energy / self.max_delta_energy
      reward = reward - self.PF * abs(current_power - self.core_budgets[core])
      if self.vector_frequencies[core]:
          reward = reward + np.mean(self.vector_frequencies[core])/self.max_freq * 2 * self.PF2 * ipc
          #print ("np.mean(self.vector_frequencies[core])/self.max_freq * 2 * ipc", np.mean(self.vector_frequencies[core])/self.max_freq * self.PF2 * ipc)
      if debug:
  ...
  def run_fast_dvfs(self, time, time_delta, new_weights, debug=False):
      ...
      for core in range(sim.config.ncores):
          ...
          if (current_power * new_weights[core][0] + ipc * new_weights[core][1]) > (self.avg_powers[core] + self.avg_ipcs[core]):
          ...
          elif (current_power * new_weights[core][0] + ipc * new_weights[core][1]) < (self.avg_powers[core] + self.avg_ipcs[core]):
              curr_freq = sim.dvfs.get_frequency(core)
              idx = self.frequencies.index(curr_freq)
