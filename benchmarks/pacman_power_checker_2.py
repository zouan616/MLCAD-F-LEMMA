...
class EnergyStats:
  def setup(self, args):
    ...
    self._dynamic_energies = []
    self._static_powers    = []
    self._dynamic_powers   = []
    for core in range(sim.config.ncores):
        self._static_energies.append(0)
        self._dynamic_energies.append(0)
        self._static_powers.append(0)
        self._dynamic_powers.append(0)
  ...
  def print_stats(self):
      print "Sim Hook End.. Displaying Final Powers and Energies of each core."
      for core in range(sim.config.ncores):
          print str(core) + ".STATIC POWER (W)", self._static_powers[core]
          print str(core) + ".DYNAMIC POWER (W)", self._dynamic_powers[core]
          print str(core) + ".STATIC ENERGY (nJ)", self._static_energies[core]
          print str(core) + ".DYNAMIC ENERGY (nJ)", self._dynamic_energies[core]
  ...
  def update_energy(self):
      ...
      if self.power and sim.stats.time() > self.time_last_energy:
        ...
        for (component, core), power in self.power.items():
          self.energy[(component, core, 'energy-static')] = self.energy.get((component, core, 'energy-static'), 0) + long(100 * power.s)
          self.energy[(component, core, 'energy-dynamic')] = self.energy.get((component, core, 'energy-dynamic'), 0) + long(100 * power.d)
          if component == 'core':
              self._static_powers[core] = power.s
              self._dynamic_powers[core] = power.d
              self._static_energies[core] = self.energy[(component, core, 'energy-static')]
              self._dynamic_energies[core] = self.energy[(component, core, 'energy-dynamic')]
              #print(len(self._static_powers),len(self._dynamic_powers),len(self._dynamic_energies),len(self._static_energies))
